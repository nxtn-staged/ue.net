...
namespace Leftice.Sdk.Tests
{
    public static class ProcessorTests
    {
        private const string DefinitionAssemblyName = "Leftice.Runtime";
        private const string ReferenceAssemblyName = "Leftice.Editor";

        private const string TestNamespace = "Leftice";
...
{{
    ...
    {{
        static class NativeMethods
        {{
            [Leftice.Processors.PointerOffset]
...
{{
    ...
    {{
        static class NativeMethods
        {{
            [System.Runtime.InteropServices.DllImport(""Leftice"")]
            static extern void Invoke();
        }}
    }}
}}
